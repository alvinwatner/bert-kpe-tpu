...
def squadkp_loader(mode, source_dataset_dir):
    ...
    #print("===== loading data... =====")
    #print(f"data_pairs[:2] = {data_pairs[:2]}")
    return data_pairs
...
def openkp_refactor(examples, mode):
    ...
    for idx, ex in enumerate(tqdm(examples)):
        ...
        if have_phrase:
            ...
            #print("===REFACTOR====")
            #print(f"data['keyphrases'] = {data['keyphrases']}")
            #print()
...
def squadkp_refactor(examples, mode):
    ...
    def tokenize_fn(text, keyphrases):
        """
        The tokenizer used in Meng et al. ACL 2017
        parse the feed-in text, filtering and tokenization
        keep [_<>,\(\)\.\'%], replace digits to 'DIGIT', split by [^a-zA-Z0-9_<>,\(\)\.\'%]
        ...
        # Modified 
        # replace the digit terms with DIGIT if it is not part of keyphrases
        extracted_tokens = []
        for i, w in enumerate(tokens):
            num_of_word_in_keywords = 0
            for keywords in keyphrases:
                for keyword in keywords:
                    if w == keyword:
                        num_of_word_in_keywords += 1

            if not re.match("^\d+$", w) or num_of_word_in_keywords > 0:                         
                extracted_tokens.append(w)
            else:
                extracted_tokens.append(DIGIT)
    ...
    for idx, ex in tqdm(enumerate(examples)):
        ...
        #src_tokens = tokenize_fn(ex['source_input'], keyphrases_tokens)
    ...
    # print("======= refactor =======")
    # print(f"return_pairs[:2] = {return_pairs[:2]}")
    return return_pairs
...
def filter_openkp_absent(examples):
    ...
    for d in data_list:           
        for k, v in d.items():
            if k == "keyphrases":                             
                print(f"keyphrases | data key = {k} | value = {v}")
            if k == "doc_words":
                print(f"doc_words | data = {k} | value = {v}")
            if k == "present_keyphrases":
                print(f"present_keyphrases | data key = {k} | value = {v}")
...
def filter_squadkp_absent(examples):
    ...
    for idx, ex in enumerate(tqdm(examples)):
        ...
        if present_phrases is None:
            ...
            #print(f"present_phrases = {present_phrases} |ex['keyphrases'] = {ex['keyphrases']} | doc_words = {ex['doc_words']}")
            #break
            continue
        if len(present_phrases['keyphrases']) != len(ex['keyphrases']):
...
if __name__ == "__main__":
    ...
    for input_mode, save_mode in mode_dir:
        #print(f"input_mode = {input_mode} | save_mode = {save_mode}")
