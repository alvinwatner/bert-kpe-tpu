...
dataset_dict = {'openkp':[('EvalPublic', 'eval'), ('Dev', 'dev'), ('Train', 'train')],
                'xsum':[('test', 'eval'), ('validation', 'dev'), ('train', 'train')],
                'cnndm':[('test', 'eval'), ('validation', 'dev'), ('train', 'train')],
                'reddit':[('test', 'eval'), ('validation', 'dev'), ('train', 'train')],
                'wikihow':[('test', 'eval'), ('validation', 'dev'), ('train', 'train')],
...
def add_preprocess_opts(parser):
    ...
    parser.add_argument("--dataset_class", type=str,
                        choices=['openkp', 'kp20k', 'xsum', 'cnndm', 'reddit', 'wikihow'],
...
def sum_loader(mode, source_dataset_dir):
    '''
    Load xsum dataset
    '''
    # mode is either train, dev, or test
    logger.info('Start loading %s data ...' % mode)
    doc_path = os.path.join(source_dataset_dir, f'{mode}.source.token')
    summ_path = os.path.join(source_dataset_dir, f'{mode}.target.token')
    doc_id_path = os.path.join(source_dataset_dir, f'{mode}.id')
    data = []
    with open(doc_path) as doc, open(summ_path) as summ, open(doc_id_path) as doc_id:
        for d, s, u in zip(doc, summ, doc_id):
            pair = {
                'source': d.strip(),
                'target': s.strip(),
                'url': u.strip()
...
def sum_refactor(examples, mode):
    logger.info('Start refactor sum %s data ... ' % mode)
    ...
    for idx, ex in tqdm(enumerate(examples)):
        src_tokens = ex['source'].strip().split()
        summ_nlp = nlp(ex['target'])
        noun_chunks = list(textacy.extract.noun_chunks(summ_nlp))
        words = list(textacy.extract.words(summ_nlp,
                                           filter_stops=True, filter_punct=True,
                                           include_pos=
                                           ['NOUN', 'PROPN', 'VERB', 'NUM']))
        entities = list(textacy.extract.entities(summ_nlp))
        doc_keyphrases = filter(lambda x: len(x) > 0, noun_chunks + words + entities)
        keyphrases = list(map(lambda x: x.split(), set(map(lambda x: x.text, doc_keyphrases))))
        data = {
            'url': ex['url'],
...
def filter_openkp_absent(examples):
    ...
    for idx, ex in enumerate(tqdm(examples)):
        ...
        present_phrases = prepro_utils.find_answer(document=lower_words,
                                                   answers=map(lambda x: x.lower(), ex['keyphrases']))
...
def filter_sum_absent(examples):
    logger.info('Start filter absent keyphrases ...')
    data_list = []
    for idx, ex in enumerate(tqdm(examples)):
        lower_words = [t.lower() for t in ex['doc_words']]
        lower_keyphrases = map(lambda x: ' '.join(x).lower().split(), ex['keyphrases'])
        present_phrases = prepro_utils.find_answer(document=lower_words,
                                                   answers=lower_keyphrases)
        if present_phrases is None:
            continue
        # dict_keyphrases = {' '.join(key).lower(): ' '.join(key) for key in ex['keyphrases']}
        # cased_keyphrases = [dict_keyphrases[' '.join(key)].split() for key in present_phrases['keyphrases']]
        data = {
            'url': ex['url'],
            'doc_words': ex['doc_words'],
            'keyphrases': ex['keyphrases'],
            'start_end_pos': present_phrases['start_end_pos'],
            'present_keyphrases': present_phrases['keyphrases']
        }
        data_list.append(data)
    return data_list
...
def main_preprocess(opt, input_mode, save_mode):
    ...
    if opt.dataset_class in ['xsum', 'cnndm', 'reddit', 'wikihow'] and save_mode != 'train':
        save_ground_truths(feed_data, gt_filename, kp_key='keyphrases')
    # -------------------------------------------------------------------------------------
...
if __name__ == "__main__":
    ...
    data_loader = {'openkp':openkp_loader, 'kp20k':kp20k_loader,
                   'xsum': sum_loader, 'cnndm': sum_loader, 'reddit': sum_loader, 'wikihow': sum_loader}
    data_refactor = {'openkp':openkp_refactor, 'kp20k':kp20k_refactor,
                     'xsum': sum_refactor, 'cnndm': sum_refactor, 'reddit': sum_refactor, 'wikihow': sum_refactor}
    absent_filter = {'openkp':filter_openkp_absent, 'kp20k':filter_kp20k_absent,
                     'xsum': filter_sum_absent, 'cnndm': filter_sum_absent, 'reddit': filter_sum_absent,
                     'wikihow': filter_sum_absent}
