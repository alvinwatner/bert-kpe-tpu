...
if __name__ == "__main__":
    ...
    if gpu:
        ...
        logger.info(
            "Process rank: %s, device: %s, num_device: %s",
            args.local_rank,
            device.type,
            device.index,
        )
   ...
   #train_sampler = (
    ...
    #model = WRAPPED_MODEL.network.to(device)
    ...
    for epoch in range(1, (args.max_train_epochs + 1)):
        ...
        #train(args, para_loader.per_device_loader(device), model, train_input_refactor, stats, tb_writer)
