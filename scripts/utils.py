...
def select_eval_script(name):
    if name == "openkp":
        ...
        return squadkp_evaluate_script, "max_f1_score3"
    elif name == "multidata":
        return multidata_evaluate_script, "max_f1_score"
    raise RuntimeError("Invalid retriever class: %s" % name)


# squadkp Evaluation Script
def squadkp_evaluate_script(args, candidate, stats, mode, metric_name="max_f1_score3"):
    logger.info("*" * 80)
    logger.info("Start Dev Evaluatng : Epoch = %d" % stats["epoch"])
    epoch_time = Timer()

    reference_filename = os.path.join(
        args.preprocess_folder, "squadkp.eval.json"
    )
